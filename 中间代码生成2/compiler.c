...
static FunctionSymbol *add_function_meta(IRGen *gen, FuncDef *func) {
    ...
    if (func->ret_type == TYPE_INT && strcmp(func->name, "main") == 0) {
        meta->llvm_name = xstrdup("__sysy_user_main");
    } else {
...
void generate_program_ir(Program *program, FILE *out) {
    ...
    for (int i = 0; i < program->items.count; ++i) {
        TopLevelItem *item = program->items.items[i];
        if (item->kind == TOP_LEVEL_FUNC &&
            item->data.func->ret_type == TYPE_INT &&
            strcmp(item->data.func->name, "main") == 0) {
            sb_append(&gen.functions, "define dso_local i32 @main() {\n");
            sb_append(&gen.functions, "entry:\n");
            sb_append(&gen.functions, "  %t_main_ret = call i32 @__sysy_user_main()\n");
            sb_append(&gen.functions, "  %t_out_state = load i32, i32* @__sysy_output_state\n");
            sb_append(&gen.functions, "  %t_need_nl = icmp eq i32 %t_out_state, 0\n");
            sb_append(&gen.functions, "  br i1 %t_need_nl, label %print_ret_nl, label %print_ret_body\n");
            sb_append(&gen.functions, "print_ret_nl:\n");
            sb_append(&gen.functions, "  call void @putch(i32 10)\n");
            sb_append(&gen.functions, "  br label %print_ret_body\n");
            sb_append(&gen.functions, "print_ret_body:\n");
            sb_append(&gen.functions, "  %t_exit_code = and i32 %t_main_ret, 255\n");
            sb_append(&gen.functions, "  call void @putint(i32 %t_exit_code)\n");
            sb_append(&gen.functions, "  ret i32 %t_main_ret\n");
            sb_append(&gen.functions, "}\n\n");
            break;
        }
    }
