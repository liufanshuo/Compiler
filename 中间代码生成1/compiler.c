...
static char *emit_global_cstr_ptr(IRGen *gen, const char *name, int len) {
    char *ptr = new_temp(gen);
    emit_func(gen, "  %s = getelementptr [%d x i8], [%d x i8]* %s, i32 0, i32 0\n",
              ptr, len, len, name);
    return ptr;
}

static void emit_print_i32(IRGen *gen, const char *value) {
    char *fmt = emit_global_cstr_ptr(gen, "@.fmt.int", 3);
    emit_func(gen, "  call i32 (i8*, ...) @printf(i8* %s, i32 %s)\n", fmt, value);
}
...
Value gen_expr(IRGen *gen, Expr *expr) {
    switch (expr->kind) {
        ...
        case EXPR_GETINT: {
            ...
            char *fmt = emit_global_cstr_ptr(gen, "@.fmt.int", 3);
            char *tmp = new_temp(gen);
            emit_func(gen, "  %s = call i32 (i8*, ...) @scanf(i8* %s, i32* %s)\n", tmp, fmt, slot);
            char *value = new_temp(gen);
            emit_func(gen, "  %s = load i32, i32* %s\n", value, slot);
            return make_value(value, 0, NULL, false, false);
        }
        case EXPR_CALL: {
            if (strcmp(expr->data.call.name, "getch") == 0) {
                char *tmp = new_temp(gen);
                emit_func(gen, "  %s = call i32 @getchar()\n", tmp);
                return make_value(tmp, 0, NULL, false, false);
            }
...
static void emit_putch(IRGen *gen, int ch) {
    emit_func(gen, "  call i32 @putchar(i32 %d)\n", ch);
}
...
void generate_program_ir(Program *program, FILE *out) {
    ...
    sb_append(&gen.globals, "@.fmt.int = private unnamed_addr constant [3 x i8] c\"%d\\00\"\n\n");
    sb_append(&gen.globals, "declare i32 @printf(i8*, ...)\n");
    sb_append(&gen.globals, "declare i32 @scanf(i8*, ...)\n");
    sb_append(&gen.globals, "declare i32 @getchar()\n");
    sb_append(&gen.globals, "declare i32 @putchar(i32)\n");
